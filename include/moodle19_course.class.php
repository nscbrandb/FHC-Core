...
class moodle19_course extends basis_db
{
	private $conn_moodle;
	public $result = array();
	...
	public function getAll($lehrveranstaltung_id, $studiensemester_kurzbz)
	{
		...
		if($result=$this->db_query($qry))
		{
			while($row = $this->db_fetch_object($result))
			{
				...
				{
					$obj->mdl_fullname = 'ID '.$obj->mdl_course_id.' Moodle v'.$obj->moodle_version;
					$obj->mdl_shortname = 'ID '.$obj->mdl_course_id.' Moodle v'.$obj->moodle_version;
				}
				$this->result[] = $obj;
			}
			return true;
...
#return true;
	...
	public function loadNoten($lehrveranstaltung_id=null, $studiensemester_kurzbz=null,$student_uid='',$bDetailinfo=false,$bServerinfo=false)
	{
		...
		UNION 
			...
			WHERE moodle_version='1.9' AND tbl_lehreinheit.lehrveranstaltung_id > 0 ";
		if ($this->lehrveranstaltung_id)
	...
	// Variable Daten Initialisieren
		...
		{
				$qry = "DELETE FROM lehre.tbl_moodle WHERE moodle_version='1.9' AND mdl_course_id='". addslashes($this->mdl_course_id) ."' ";
