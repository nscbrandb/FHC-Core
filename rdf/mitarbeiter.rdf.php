 ...
 */
	...
	$lektor = optional_param('lektor',null,PARAM_CONVBOOL);
	$fixangestellt = optional_param('fixangestellt',null,PARAM_CONVBOOL);
	$stg_kz = optional_param('stg_kz',null,PARAM_INT);
	$fachbereich_id = optional_param('fachbereich_id',null,PARAM_INT);
	$user = optional_param('user',false,PARAM_CONVBOOL);
	$filter = optional_param('filter',null,PARAM_NOTAGS);
	$mitarbeiter_uid = optional_param('mitarbeiter_uid',null,PARAM_NOTAGS);	
	$aktiv = optional_param('aktiv',false,PARAM_CONVBOOL);
	$lehrveranstaltung_id = optional_param('lehrveranstaltung_id',null,PARAM_INT);
	$filter2 = optional_param('filter2',null,PARAM_NOTAGS);
	...
	function checkfilter($row, $filter2) {
		...
		if($filter2=='konto') {
			...
			if($db->db_query($qry)) {
				if($row_filter = $db->db_fetch_object()) {
					if($row_filter->anzahl>0 ) return false;
				}
			}
		}
		return true;
	...
	if($lehrveranstaltung_id == null && $filter == null && $mitarbeiter_uid == null) {
		...
		foreach ($stg_obj->result as $stg) $stg_arr[$stg->studiengang_kz] = $stg->kuerzel;
		
		$alle = '';
		foreach ($ma as $mitarbeiter) {
			if ($aktiv && $mitarbeiter->aktiv != true) continue;
			else {
		...
		$desc= '
				<RDF:Description about="'.$rdf_url.'_alle" >
					...
					<MITARBEITER:aktivbild></MITARBEITER:aktivbild>
		...
		if ($user) {
			...
			if($bb->getBerechtigungen(get_uid())) {
				...
				foreach ($ma as $mitarbeiter) {
					...
					if ($mitarbeiter->studiengang_kz < 100) continue;
		...
		if(isset($_GET['optional']) && $_GET['optional'] == 'true') {
			echo '
				...
				<RDF:Description about="" >
					...
					<MITARBEITER:aktiv><![CDATA[]]></MITARBEITER:aktiv>
					<MITARBEITER:aktivbild><![CDATA[]]></MITARBEITER:aktivbild>
					<MITARBEITER:studiengang_kz></MITARBEITER:studiengang_kz>
			  	</RDF:Description>
			  	</RDF:li>
			  	';
		...
		if($mitarbeiter_uid != null) {
			...
			if($filter == null) $mitarbeiter->getMitarbeiterFromLehrveranstaltung($lehrveranstaltung_id);
			else $mitarbeiter->getMitarbeiterFilter($filter);
	...
	function draw_row($mitarbeiter)	{
		...
		<RDF:Description about="'.$rdf_url.$mitarbeiter->uid.'" >
			...
			<MITARBEITER:aktiv><![CDATA['.($mitarbeiter->aktiv?'true':'false').']]></MITARBEITER:aktiv>
			<MITARBEITER:aktivbild><![CDATA['.($mitarbeiter->aktiv?'../../addons/STPCore/skin/images/onbe_ok.png':'../../addons/STPCore/skin/images/onbe_open.png').']]></MITARBEITER:aktivbild>
			<MITARBEITER:studiengang_kz></MITARBEITER:studiengang_kz>
	  	</RDF:Description>
	  	';
	}
